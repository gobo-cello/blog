...
- plugin が production mode で追跡しない設定・ビルド専用の依存を `ignoreDependencies` に足す場合は、先に plugin の対応状況と、設定・ビルドファイルの明示的な `entry` 化で解消できないかを確認する（「zero config から外れる場合」を参照）。
...
- 設定はリポジトリ直下の `biome.json` 一つで、`infra/`・`app/`・`e2e/` を含むリポジトリ全体をカバーする。各ディレクトリに本当に異なる責務・言語・ライフサイクルが生じない限り nested configuration(`extends: "//"` を含む)は置かない。設定の現状は `biome.json` を参照する。
