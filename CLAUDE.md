...
- 設定はリポジトリ直下の `biome.json` 一つで、`infra/`・`app/`・`e2e/` を含むリポジトリ全体をカバーする。各ディレクトリに本当に異なる責務・言語・ライフサイクルが生じない限り nested configuration(`extends: "//"` を含む)は置かない。現在の `biome.json` は `vcs` 連携、`css.parser.tailwindDirectives`、および `overrides` を持つ。
