# 0003: blog.example.comのhosted zoneと証明書
...
### ドメイン名は`blog.example.com`とする

apex(`example.com`)は今回使用せず、将来別のサブドメインが
増える余地として空けておく。

### hosted zoneはblog-production accountが所有する

`blog.example.com`のhosted zoneは、このリポジトリのCDKで
...
- 将来`sandbox.blog.example.com`(フェーズG)を追加する場合は、
  `blog.example.com`のhosted zoneから`blog-sandbox` accountへ
