...
10. deploy出力の`SandboxBlogHostedZoneNameServers`を`infra/.env.local`の`SANDBOX_SUBDOMAIN_NAME_SERVERS`に設定し、`ProductionDnsStack`を再deployします。証明書のDNS検証は、委譲が反映されるまで完了を待つ場合があります。
