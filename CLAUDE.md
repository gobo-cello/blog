...
ブログアプリケーション・コンテンツ・ワークロード用インフラを管理するリポジトリ。

## このリポジトリで管理するもの

- ブログのアプリケーションコードとコンテンツ
- ブログの本番(`blog-production`)・検証(`blog-sandbox`)ワークロード用 Infrastructure as Code(`infra/`)
- ブログのデプロイに関する GitHub Actions と AWS の OIDC 連携
...
- ドメイン名やサイト名に依存する値は、環境変数、GitHub Variables/Secrets、CDK context、設定ファイル、CMSのコンテンツなど、コード外の設定として注入すること。コード側はそれらを汎用的なパラメータとして受け取るように書くこと。
...
- **`workspaces`。** `.` / `app` / `infra` / `e2e` を解析単位として宣言し、`treatConfigHintsAsErrors: true` を設定する。
- **`app`（Astro）。** Astro plugin が production mode で追跡しない設定・ビルド専用の依存（`zod`、`mermaid` / `astro-mermaid`、`@expressive-code/core` / `astro-expressive-code`、`@astrojs/sitemap`、`@tailwindcss/vite`、`vite`）を `ignoreDependencies` に末尾 `!` 付きで列挙し、production mode でのみ除外する。`project` は `scripts/check-unused-images.ts`（`check:images` から `tsx` 実行）と `src/lib/code-block-title.ts` を production mode でのみ対象外にする。入口は Astro plugin が解決するため `entry` の追加はない。
- **`infra`（CDK）。** 上の「CDK と `tsx`」のとおり `entry: ["bin/infra.ts!"]` と `ignoreDependencies: ["tsx"]` を置く。
- **`e2e`。** Playwright plugin が入口を解決するため追加設定なし（`{}`）。
- `app` の `ignoreDependencies` が増える場合は、Astro plugin の対応状況を確認し、設定・ビルドファイルの明示的な `entry` 化で解消できないかを先に検討する（「zero config から外れる場合」を参照）。
...
- 設定はリポジトリ直下の `biome.json` 一つで、`infra/`・`app/`・`e2e/` を含むリポジトリ全体をカバーする。各ディレクトリに本当に異なる責務・言語・ライフサイクルが生じない限り nested configuration(`extends: "//"` を含む)は置かない。現在の `biome.json` は `vcs` 連携、`css.parser.tailwindDirectives`、`linter`(下記)、および `overrides` を持つ。
...
- `vcs.useIgnoreFile` で `.gitignore` を尊重する。`infra/cdk.out/` や `app` のビルド出力(`app/build/`・`app/.react-router/` など)といった生成物は解析対象に含めない。ただし、除外で診断数が減ったことだけを成功とみなさず、実行時に必要なファイルまで除外していないか確認する。
- glob とパスは `biome.json` を基準に解決される。`*` と `**` の違い、否定パターンの順序、ディレクトリを対象にする記法を、シェルの glob と混同しない。

### overrides と suppression は狭く保つ

- 現在の `overrides` は、`**/*.html` の formatter 設定(`selfCloseVoidElements`)と、`**/*.astro` に対する未使用 import／変数ルールの無効化。いずれも明確な境界を持つ例外の範囲にとどめ、新たな `overrides` を広い glob で通常の source 全体へ広げない。適用順序と例外の理由を追跡可能にする。
...
- 型の正しさは TypeScript が source of truth とする(型チェックは `infra` の `npm run build`、`app` の `npm run check:types`、`e2e` の `npm run check:types`)。Biome の lint と競合する場合は型を優先し、必要なら対象の狭い `overrides` または suppression で調整する。
- 未使用のファイル・export・依存は Knip、テストは Vitest(`infra`／`app`)と Playwright(`e2e`)、ワークフローの検査は actionlint が担当する。これらを Biome で代替せず、重複診断や相反する自動修正が生じたら、どちらを source of truth とするかを決めて片方を狭める。
